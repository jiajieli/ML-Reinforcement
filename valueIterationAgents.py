...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for n in range(iterations):
            for s in mdp.getStates():
                self.computeActionFromValues(s)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for s_, p in self.mdp.getTransitionStatesAndProbs(state, action):
            if self.mdp.isTerminal(s_):
                QValue += p * self.mdp.getReward(state, action, s_)
            else:
                QValue += p * (self.mdp.getReward(state, action, s_) + self.discount
                    * self.getValue(s_))
        return QValue
